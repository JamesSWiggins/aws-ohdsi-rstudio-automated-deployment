#### Works on EC2 with EMR 5.12 AMI
#!/bin/bash
...
sudo yum install R-core R R-core-devel R-devel xorg-x11-xauth.x86_64 xorg-x11-server-utils.x86_64 xterm libXt libX11-devel libXt-devel libcurl-devel git compat-gmp4 compat-libffi5 openssl-devel -y
RSTUDIO_FILE=$(basename $RSTUDIO_URL)
wget $RSTUDIO_URL
sudo yum install —nogpgcheck -y $RSTUDIO_FILE
SHINY_FILE=$(basename $SHINY_URL)
